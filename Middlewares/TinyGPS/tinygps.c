...
void gps_get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
	...
	if (fix_age)
	{
		*fix_age = (_last_position_fix == GPS_INVALID_FIX_TIME) ? GPS_INVALID_AGE : uptime() - _last_position_fix;
	}
}
...
void gps_get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
	...
	if (age)
	{
		*age = _last_time_fix == GPS_INVALID_FIX_TIME ? GPS_INVALID_AGE : uptime() - _last_time_fix;
	}
}
