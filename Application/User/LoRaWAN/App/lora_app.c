...
static void SendTxDataCustom(void);
...
void LoRaWAN_Init(void) {
	...
	UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_LoRaSendCustom), UTIL_SEQ_RFU,
			SendTxDataCustom);
	...
	// We start the LoRaMAC layer in order to get a proper status of the join
	LoRaMacStart();
	...
	if (LmHandlerJoinStatus() != LORAMAC_HANDLER_SET) {
		...
		// Join Status is not set, so we stop the LoRaMAC since it will be restarted again during the join procedure
		LoRaMacStop();

		// Call the join request
	...
	UTIL_TIMER_Create(&TxTimer, 0xFFFFFFFFU, UTIL_TIMER_ONESHOT, OnTxTimerEvent,
	NULL);
	UTIL_TIMER_SetPeriod(&TxTimer, APP_TX_DUTYCYCLE);
	UTIL_TIMER_Start(&TxTimer);

	/* Tx the Standard frame */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	bool l_bNvMResetStatus = false;

	switch (GPIO_Pin) {
	case BUTTON_SW1_PIN: {
		// We stop the timer to avoid Tx collision
		...
		break;
	case BUTTON_SW2_PIN: {
		l_bNvMResetStatus = NvmDataMgmtFactoryReset();

		if (true != l_bNvMResetStatus) {
			BSP_LED_On(LED_RED);
			APP_LOG(TS_OFF, VLEVEL_M,
	...
	case BUTTON_SW3_PIN: {
		...
		/* Tx the Custom frame */
		UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_LoRaSendCustom), CFG_SEQ_Prio_0);
...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params) {
	...
	if ((appData != NULL) || (params != NULL)) {
		...
		case CAYENNE_DIGITAL_IO_APP_PORT:

			if (appData->BufferSize != 0) {
				if (appData->Buffer[0] == 0x04) {

					if (appData->Buffer[2] != 0) {
						APP_LOG(TS_OFF, VLEVEL_H, "RED LED ON\r\n");
						BSP_LED_On(LED_RED);
					} else {
						APP_LOG(TS_OFF, VLEVEL_H, "RED LED Off\r\n");
						BSP_LED_Off(LED_RED);
					}
				}
...
AppData.Buffer[i++] = (uint8_t) 'E';
AppData.Buffer[i++] = (uint8_t) 'S';
AppData.Buffer[i++] = (uint8_t) 'E';
...
if (LORAMAC_HANDLER_SUCCESS
		== LmHandlerSend(&AppData, LORAWAN_DEFAULT_CONFIRMED_MSG_STATE,
				&nextTxIn, false)) {
	APP_LOG(TS_ON, VLEVEL_L, "CUSTOM SEND REQUEST\r\n");
} else if (nextTxIn > 0) {
...
latitude = (int32_t) (sensor_data.latitude);
longitude = (int32_t) (sensor_data.longitude);
...
CayenneLppAddGps(channel++, latitude, longitude, 0);
